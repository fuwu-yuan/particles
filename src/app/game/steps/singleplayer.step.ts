...
export class SingleplayerStep extends GameStep {
  ...
  constructor(board: Board) {
    ...
    this.timerLabel.fontFamily = 'KenvectorFutureThin';
    ...
    this.bestScore.fontFamily = 'KenvectorFutureThin';
  ...
  onEnter(data: any): void {
    ...
    const start = new Entities.Button(
      ...
      'CLICK TO START');
    start.fontSize = 25;
    start.fontFamily = 'KenvectorFutureRegular';
  ...
  addEnnemy(x?: number, y?: number, speed?: number, angle?: number): Ennemy {
    const ennemy = new Ennemy(
      x || SingleplayerStep.random(WALL_WIDTH + BALLS_RADIUS + 10, this.board.width - BALLS_RADIUS - WALL_WIDTH - 50),
      y || SingleplayerStep.random(WALL_WIDTH + BALLS_RADIUS + 10, this.board.height - BALLS_RADIUS - WALL_WIDTH - 50),
      BALLS_RADIUS);
    ennemy.stopped = true;
    this.board.addTimer(BALL_START_TIMER * 1000, () => {
      ennemy.setSpeedWithAngle(
        speed || SingleplayerStep.random(BALLS_SPEED.MIN, BALLS_SPEED.MAX),
        angle || SingleplayerStep.random(0, 359), true
      );
  ...
  private playerLoose(): void {
    ...
    this.board.addTimer(3000, () => {
      ...
      top10Title.fontFamily = 'KenvectorFutureRegular';
      top10Title.fontSize = 25;
      ...
      this.addScore(this.elapsedMs, this.ennemies.length).then(() => {
        ...
        for (let i = 0; i < top10values.length; i++) {
          const top10Label = new Entities.Label(
            ...
            this.board.ctx);
          top10Label.fontFamily = 'KenvectorFutureThin';
          if (Plateform.isTouchScreen()) {
            top10Label.fontSize = 12;
          }
      ...
      restart.fontFamily = 'KenvectorFutureThin';
      ...
      score.fontFamily = 'KenvectorFutureThin';
      score.fontSize = 20;
